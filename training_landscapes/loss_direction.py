# Content: Training landscapes via sampling different directions 
...
def calculation_process(cir, param, param_change, lam_list, target_state):
    ...
    # update to the specific direction, i.e. theta + lambda * new_theta
    for lam in lam_list:
        lam_param = []
...
def loss_sample_lambda(input_cir, input_u, sample_num: int, param, overlap):
    ...
    for _ in tqdm(range(sample_num)):
        ...
        loss_l = calculation_process(input_cir, param, new_param, lam_list=lam_range, target_state = target_state)
    ...
    np.savetxt(f"./data1/loss_direction_n{N}_p{overlap}_sub_depth{depth}_sample{sample_num}.txt", total_loss)
    return
...
if __name__ == "__main__":
    ...
    sample = 1 # Number of sampled random states
